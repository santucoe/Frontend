...
namespace PlanDigitization_web.Controllers
{
    ...
    public class UserSettingsController : Controller
    {
        ...
        {
            if (this.HasPermission("GatewayDetails-View"))
            {
                if (Session["CompanyCode"] != null)
                {
                    return View();
        ...
        {
            if (this.HasPermission("GatewayDetails-Edit"))
            {
                if (Session["CompanyCode"] != null)
                {
                try
        ...
        public async Task<ActionResult> Break_settings(Models.usersettings U)
        {
            ...
            if (this.HasPermission("BreakSettings-View"))
            {
                if (Session["CompanyCode"] != null || Session["PlantCode"] != null)
                {
                    try
                        ...
                        if (postedFile != null)
                        {
                            if (ModelState.IsValid)
                            {
                                ...
                                if (filename.EndsWith(".xlsx") || filename.EndsWith(".xls") || filename.EndsWith(".csv"))
                                {
                                    ...
                                    string newfile = DateTime.Now.ToString("MM_dd_yyyy_hh_mm_ss") + "_" + postedFile.FileName;
                                    string path = Server.MapPath("~/upload_excel/" + newfile);
                                    postedFile.SaveAs(path);
                                    ...
                                    if (dReader.HasRows)
                                    {
                                        ...
                                        while (dReader.Read())
                                        {
                                            ...
                                            if (dReader["Alarm_ID"] != DBNull.Value)
                                            {
        ...
        public void DlteUploadexcl()
        {
            DirectoryInfo d = new DirectoryInfo(Path.Combine(System.Web.HttpContext.Current.Server.MapPath("~/upload_excel/")));//Assuming Test is your Folder
